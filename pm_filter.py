...
async def leaveassoom(_, m):
    if m.chat.id not in (-1001978038633, -1001627975184, -1001799344460, -1001524622686, -1001814841940, -1001774867816, -1001957960861, -1001915385042):
        try:
            await m.reply_text("It's a premium bot if u want to use it Please take premium subscription from: @Akash5213")
            await m.chat.leave()
        except:
            pass
...
# ------------------------------------------------------------------------------------------------- #
        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                ...
                logger.info("PM Filter: Sending file to log channel")
                ...
                reply_markup = InlineKeyboardMarkup([[
                    InlineKeyboardButton('🚀 𝐅𝐚𝐬𝐭 𝐃𝐨𝐰𝐧𝐥𝐨𝐚𝐝 🚀', url=f"{STREAM_URL}/{abs(STREAM_CHANNEL)}/{log_msg.id}"),
                    InlineKeyboardButton('⚡ 𝐒𝐭𝐫𝐞𝐚𝐦 𝐋𝐢𝐧𝐤 ⚡', url=f"{STREAM_URL}/{abs(STREAM_CHANNEL)}/{log_msg.id}")
                ]])
...
async def auto_filter(client, message):
    srh_msg = await message.reply_text(f"<b>sᴇᴀʀᴄʜɪɴɢ ғᴏʀ {message.text}🔎</b>")
    ...
    if len(message.text) < 100:
        ...
        if not files:
            ...
            search = re_search
            await srh_msg.edit_text(f"<b>ᴘʟᴇᴀsᴇ ᴡᴀɪᴛ {re_search} 🔎</b>")
            ...
            if not files:
                ...
                search = re_search_
                await srh_msg.edit_text(f"<b>ᴘʟᴇᴀsᴇ ᴡᴀɪᴛ More {re_search_} 🔎</b>")
...
async def advantage_spell_chok(client, message, srh_msg):
    srh_msg = await srh_msg.edit_text("<b>ᴄʜᴇᴄᴋɪɴɢ ʏᴏᴜʀ sᴘᴇʟʟɪɴɢ...🔎</b>")
    ...
    if k == False:
        ...
        if not files and len(words) > 1:
            await srh_msg.edit_text("<b>ᴄʜᴇᴄᴋɪɴɢ ʏᴏᴜʀ sᴘᴇʟʟɪɴɢ V2...🔎</b>")
